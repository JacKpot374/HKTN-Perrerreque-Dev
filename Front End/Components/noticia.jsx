    ...
    <div className="p-6 flex flex-col">
      <div className="mb-6 animate-slide-up stagger-1">
        <h2 className="text-2xl font-bold mb-1" style={{ color: COLORS.blue, fontFamily: 'Krub, sans-serif' }}>
          {t.noticiasTitulo || es.noticiasTitulo || 'Noticias Locales'}
        </h2>
      </div>
        ...
        {NEWS.map((item, index) => (
          <div key={item.id} className={`bg-white rounded-3xl overflow-hidden shadow-md animate-slide-up stagger-${(index % 3) + 1}`}>
            <div className="h-40 overflow-hidden relative">
              ...
              <div className="absolute top-3 left-3 bg-white/90 px-3 py-1 rounded-full text-[10px] font-black uppercase" style={{ color: COLORS.blue }}>
                {traducirTag(item.tag)}
              </div>
            </div>
            ...
            <div className="p-5">
              ...
              {/* MAGIA DE TRADUCCIÓN: Buscamos el título de la noticia en el diccionario usando su ID */}
              <h3 className="font-bold text-lg leading-tight mb-3">
                {t[`noticia_${item.id}_titulo`] || item.title}
              </h3>
              
              <button className="text-sm font-bold flex items-center gap-1" style={{ color: COLORS.green }}>
                {t.leerMas || es.leerMas || 'Leer más'} <ChevronRight size={16} />
              </button>
            </div>
          </div>
        ))}
